...
interface OrderItem { name: string; quantity: number; price: number; variant?: string; }
...
export const emailService = {
  ...
  async sendOrderConfirmation(to: string, data: {
    ...
    const rows = data.items.map(i =>
      `<div class="row"><span>${i.name}${i.variant ? ` · ${i.variant}` : ''} ×${i.quantity}</span><span>€${(i.price * i.quantity).toFixed(2)}</span></div>`
    ).join('');
  ...
  async sendDeliveryConfirmation(to: string, data: { customerName: string; orderId: string }) {
    await transporter.sendMail({
      from:    `"PrimeShop" <${process.env.SMTP_USER}>`,
      to,
  ...
  async sendWelcomeEmail(to: string, data: { name: string }) {
    await transporter.sendMail({
      from:    `"PrimeShop" <${process.env.SMTP_USER}>`,
      to,
